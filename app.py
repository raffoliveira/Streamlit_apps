...
data = pd.read_csv('./data/estoque.csv')

st.title('Data analysis')
st.write('In this project we gonna analyse the amount of products in our'
...
if checkbox_mostrar_tabela:
    ...
    if categoria != 'Todas':
        df_categoria = data.query('Categoria == @categoria')
        mostra_qntd_linhas(df_categoria)
    else:
        mostra_qntd_linhas(data)

# filtro para o gráfico
st.sidebar.markdown('## Filtro para o gráfico')

categoria_grafico = st.sidebar.selectbox(
    'Selecione a categoria para apresentar no gráfico', options=data['Categoria'].unique())
figura = plot_estoque(data, categoria_grafico)
st.pyplot(figura)
